...
msra_root_dir='C:/Users/86159/data/MSRA-TD500/'
...
hust_root_dir='C:/Users/86159/data/HUST-TR400/'
...
def get_ann(img,gt_path):
    h,w=img.shape[0:2]
    #读取格式print(gt_path)C:/Users/86159/data/TD500/Train/0.txt
    lines=mmcv.list_from_file(gt_path)
    bboxes=[]
    words=[]
    for line in lines:
        line=line.encode('utf-8').decode('utf-8-sig')
        #746,861,793,858,798,963,751,966,undefined
        line=line.replace('\xef\xbb\xbf','')

        gt=line.split(' ')
        #['0', '0', '749', '860', '47', '105', '-0.048040']
        #编号 难度  x y(左上) w h theta
        ...
        bbox=cv2.boxPoints(((x1,y1),(w_,h_),theta))#可获取该矩形的四个顶点坐标。
        # 获取最小外接矩阵，中心点坐标，宽高，旋转角度
        bbox=bbox.reshape(-1)/([w*1.0,h*1.0]*4)
        #[751.54821777 966.06793213 746.50598145 861.18902588 793.45178223
         #858.93206787 798.49401855 963.81097412]/(w,h)
        '''
[[751.5482  966.06793]
 [746.506   861.189  ]
 [793.4518  858.93207]
...
class PAN_MSRA(data.Dataset):
    def __init__(self,
        ...
        if split=='train':
            ...
            gt_dirs=[msra_train_gt_dir,msra_train_gt_dir]
        elif split=='test':
    ...
    def prepare_train_data(self,index):
        ...
        gt_insatnce=np.zeros(img.shape[0:2],dtype='uint8')
        training_mask=np.ones(img.shape[0:2],dtype='uint8')

        if bboxes.shape[0]>0:
            bboxes=np.reshape(bboxes*([img.shape[1],img.shape[0]]*4),(bboxes.shape[0],-1,2)).astype('int32')

            for i in range(bboxes.shape[0]):
                cv2.drawContours(gt_insatnce,[bboxes[i]],-1,i+1,-1)#文本框实例 gt_instance i+1
                if words[i]=='###':
                    cv2.drawContours(training_mask,[bboxes[i]],-1,0,-1)#掩模
        ...
        for rate in [self.kernel_scale]:
            ...
            for i in range(bboxes.shape[0]):
                cv2.drawContours(gt_kernel,[kernel_bboxes[i]],-1,1,-1)#gt_kenrel shrink后的文本框 1
            gt_kernels.append(gt_kernel)

        if self.is_transform:
            imgs=[img,gt_insatnce,training_mask]
            imgs.extend(gt_kernels)

            imgs=random_horizontal_flip(imgs)
            imgs=random_rotate(imgs)
            imgs=random_crop_padding(imgs,self.img_size)
            img,gt_insatnce,training_mask,gt_kernels=imgs[0],imgs[1],imgs[2],imgs[3:]

        gt_text=gt_insatnce.copy()#文本框实例 gt_instance i+1
        gt_text[gt_text>0]=1#0/1处理后的文本框实例 gt_text 1
        gt_kernels=np.array(gt_kernels)

        max_instance=np.max(gt_insatnce)
        ...
        for i in range(1,max_instance+1):
            ...
            gt_bboxes[i]=(tl[0],tl[1],br[0],br[1])#存放bboxes的最小外包矩形的四个顶点
        ...
        gt_text=torch.from_numpy(gt_text).long()#0/1处理后的文本框实例 gt_text 1
        gt_kernels=torch.from_numpy(gt_kernels).long()#gt_kenrel shrink后的文本框 1
        training_mask=torch.from_numpy(training_mask).long()
        gt_insatnce=torch.from_numpy(gt_insatnce).long()#文本框实例 gt_instance i+1
        gt_bboxes=torch.from_numpy(gt_bboxes).long()#存放bboxes的最小外包矩形的四个顶点
