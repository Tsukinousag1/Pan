...
ctw_root_dir='/mnt/disk2/std2021/hejiabang-data/OCR/ctw1500/'
ctw_train_data_dir=ctw_root_dir+'train/text_image/'
ctw_train_gt_dir=ctw_root_dir+'train/text_label_circum/'
...
class PAN_CTW(data.Dataset):
    def __init__(self,
        ...
        self.img_size=img_size if (
            img_size is None or isinstance(img_size,tuple)) else (img_size,img_size)
    ...
    def prepare_train_data(self,index):
        ...
        if len(bboxes)>0:
            for i in range(len(bboxes)):
                bboxes[i]=np.reshape(
                    bboxes[i]*([img.shape[1],img.shape[0]]*(bboxes[i].shape[0]//2)),
        ...
        max_instance=np.max(gt_instance)#最大实例
        ...
        for i in range(1,max_instance+1):
            ...
            tl=np.min(points,axis=0)#竖的方向最小值
        ...
        gt_text=torch.from_numpy(gt_text).long()#gt_instance 0/1化处理的结果
        gt_kernels=torch.from_numpy(gt_kernels).long()#shrink后的gt
        training_mask=torch.from_numpy(training_mask).long()#掩膜用于去除‘  ###’
        gt_instance=torch.from_numpy(gt_instance).long()#文本实例
        gt_bboxes=torch.from_numpy(gt_bboxes).long()#存bbox的四点min min max max

        data=dict(
            imgs=img,
            gt_texts=gt_text,#gt_instance 0/1化处理的结果
            gt_kernels=gt_kernels,#shrink后的gt
            training_masks=training_mask,#掩膜用于去除‘  ###’
            gt_instances=gt_instance,#文本实例
            gt_bboxes=gt_bboxes,#存bbox的四点min min max max
            #train时多添加一个cfg=cfg
        )
...
a=np.array([[1,0,1],
            [0,1,0],
            [1,0,1]])
points=np.array(np.where(a)).transpose((1,0))
tl=np.min(points,axis=1)
