...
def random_rotate(imgs):
    max_single=10
    angle=random.random()*2*max_single-max_single
    for i in range(len(imgs)):
        img=imgs[i]
        w,h=img.shape[:2]
        rotate_matrix=cv2.getRotationMatrix2D((h/2,w/2),angle,1)
...
def random_scale(img,short_size=756):
    h,w=img.shape[0:2]

    scale=np.random.choice(np.array([0.7,0.8,0.9,1.0,1.1,1.2,1.3]))
    scale=(scale*short_size)/min(w,h)
